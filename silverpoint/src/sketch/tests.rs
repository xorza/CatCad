...
/// Nearness is a disc of `DUPLICATE_EPSILON`, and a duplicate that something is
...
fn the_cleanup_measures_nearness_and_spares_what_is_spoken_for() {
    ...
    let near = sketch.add_point(DVec2::new(DUPLICATE_EPSILON * 0.5, 0.0));
    let far = sketch.add_point(DVec2::new(DUPLICATE_EPSILON * 2.0, 0.0));
