use crate::math::quadratic::{grazing_roots, roots};
...
/// The grazing entry differs from [`roots`] in the double root and in
/// nothing else.
///
...
fn a_graze_is_the_double_root_it_stands_for() {
    assert_eq!(grazing_roots(1.0, -4.0, 4.0), Some([2.0, 2.0]));
    assert_eq!(grazing_roots(1.0, 0.0, 0.0), Some([0.0, 0.0]));
    assert_eq!(grazing_roots(3.0, 0.0, 0.0), Some([0.0, 0.0]));
    // A real crossing pair, and a miss, are what `roots` says they are.
    assert_eq!(grazing_roots(1.0, -3.0, 2.0), roots(1.0, -3.0, 2.0));
    assert_eq!(grazing_roots(1.0, 0.0, 1.0), None);
    assert_eq!(grazing_roots(0.0, 2.0, -1.0), None);
}
