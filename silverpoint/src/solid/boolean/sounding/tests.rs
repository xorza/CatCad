...
/// places on its own faces rather than guessing.
///
...
fn a_block_holds_what_is_within_it_and_owns_its_own_faces() {
    ...
    // On the base, on the far end, and on a wall.
    assert_eq!(standing(&body, (1.0, 1.0), 0.0), Standing::On);
    assert_eq!(standing(&body, (1.0, 1.0), 3.0), Standing::On);
    assert_eq!(standing(&body, (0.0, 1.0), 1.5), Standing::On);
...
fn a_place_down_a_bore_is_outside_the_block_it_is_bored_through() {
    ...
    // On the wall of the bore, which is a face like any other.
    assert_eq!(standing(&body, (2.0, 3.0), 2.5), Standing::On);
    // Above the bore, level with the far end: clear of the solid either way.
    assert_eq!(standing(&body, (3.0, 3.0), 7.0), Standing::Outside);
}

/// A block grown the other way off its plane is sounded the same.
///
