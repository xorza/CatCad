...
use crate::solid::geometry::quartic::{Closing, Quartic, Stretch};
...
fn an_ellipse_reads_its_parameter_back_and_not_its_bearing() {
    ...
    let place = curve.at(eighth, &carried);
    ...
    for step in 0..8 {
        ...
        let read = curve.along(curve.at(t, &carried), &carried);
        assert!((read - t).abs() < 1e-12, "{t} read back as {read}");
        assert!(
            curve
                .at(read, &carried)
                .abs_diff_eq(curve.at(t, &carried), 1e-12)
        );
    }
}
...
fn a_quadric_diagonalizes_by_congruence_and_says_how_it_leans() {
    ...
    // The member the pencil's own search settled on, held ruled by the route
    // that search does not take.
    assert!(
        Congruence::of(&member).signature().ruled(),
        "the member the pencil found is not ruled"
    );
...
/// being irrational.
///
/// **A sphere holds no line**, which is `None` here and is the same fact
/// [`Signature::ruled`] reports about the surface as a whole. Held over four of
/// its places, so that it is the surface answering and not one awkward place on
/// it.
///
...
fn a_marched_curve_answers_through_the_store_it_names() {
    ...
    let run = carried.marched.add(&walked, 1e-3);
    ...
    assert!(curve.at(0.0, &carried).abs_diff_eq(DVec3::X, 1e-12));
...
///
/// **A second answer to the one question the algebraic route asks.**
/// [`Quartic::of`](crate::solid::geometry::quartic::Quartic) decides a member
/// ruled by the sign of its rulings' own discriminant, and never asks for a
/// signature. This is here so that the two routes can be held against each
