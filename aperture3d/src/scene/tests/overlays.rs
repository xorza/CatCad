...
fn the_extent_covers_transformed_meshes_and_curves() {
    ...
    // thing in it — see [`Scene::cover`]. Reaching far enough to be unmissable
    // in every direction, and in both a mesh batch and an overlay batch,
    // because a walk that forgot one of them would still pass a test that used
    // the other.
