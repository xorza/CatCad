...
impl SceneView {
    ...
    pub(crate) fn stands(
        ...
        match about {
            ...
            // holds a profile at all: the arrangement it was read from is not
            // the one it is being drawn against.
            Asking::Extrude { profile, .. } | Asking::Revolve { profile, .. } => self
                .picture
                .grown_footprint(models, profile, lens)
                .map(Stands::Beside),
        }
    }
}
