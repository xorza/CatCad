...
pub(super) fn show(ui: &mut Ui, cube: &mut Cube, shown: Shown<'_>, intents: &mut Intents) {
    ...
    Panel::vstack()
        ...
        .size((Sizing::fixed(cube::BOX), Sizing::HUG))
        .gap(look::GAP)
        .background(Background::NONE)
        .show(ui, |ui| {
            cube.show(ui, camera_id("cube"), camera, intents);
            Pill::hstack("view").show(ui, |ui| {
                if Chip::icon(camera_id("Projection"), tip, glyph).show(ui, shown.icons) {
                    intents.push(Change::Project(projection.toggled()));
                }
                if Chip::icon(camera_id("Fit"), "Frame the whole model", Glyph::Fit)
                    .show(ui, shown.icons)
                {
