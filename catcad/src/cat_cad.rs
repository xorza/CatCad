...
impl CatCad {
    ...
    pub(crate) fn write(&mut self, path: PathBuf) {
    ...
    /// [`Document::open`].
    pub(crate) fn read(&mut self, path: PathBuf) {
    ...
    pub(crate) fn status(&self) -> Status<'_> {
        let models = self.document.models(&self.build, self.session.editing());
...
pub(crate) mod internals {
    ...
    impl CatCad {
        ...
        /// one line.
        #[cfg(test)]
        pub(crate) fn models(&self) -> crate::model::models::Models<'_> {
            self.document.models(&self.build, self.session.editing())
        }
